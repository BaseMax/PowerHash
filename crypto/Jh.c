...
static void F8(hashState *state)
{
	...
	state->x[4][0] ^= ((uint64_t*)state->buffer)[0];
	state->x[4][1] ^= ((uint64_t*)state->buffer)[1];
	state->x[5][0] ^= ((uint64_t*)state->buffer)[2];
	state->x[5][1] ^= ((uint64_t*)state->buffer)[3];
	state->x[6][0] ^= ((uint64_t*)state->buffer)[4];
	state->x[6][1] ^= ((uint64_t*)state->buffer)[5];
	state->x[7][0] ^= ((uint64_t*)state->buffer)[6];
	state->x[7][1] ^= ((uint64_t*)state->buffer)[7];
}
void jh_hash(const unsigned char *data,unsigned char *hashval)
{
	hashState state;
	...
	memset(state.buffer+8,0,56);
	state.buffer[8] |= 128;
	F8(&state);
	memset(state.buffer,0,62);//62 also can be 50
	state.buffer[63] = 64;
	state.buffer[62] = 6;
	F8(&state);
	memcpy(hashval,(unsigned char*)state.x+96,32);
}
